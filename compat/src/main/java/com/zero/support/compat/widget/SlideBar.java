...
public class SlideBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ZeroSlideBar);

        textSize = a.getDimensionPixelSize(R.styleable.ZeroSlideBar_android_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
        defaultColor = a.getColor(R.styleable.ZeroSlideBar_android_textColor, Color.BLACK);
        chooseColor = a.getColor(R.styleable.ZeroSlideBar_slideBar_chooseTextColor, Color.MAGENTA);
        chooseBackgroundColor = a.getColor(R.styleable.ZeroSlideBar_slideBar_chooseBackgroundColor, Color.LTGRAY);
        chooseStyle = STYLE.getFromInt(a.getInt(R.styleable.ZeroSlideBar_slideBar_style, 0));
        a.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            //画字母
            canvas.drawText(letters[i], x, y - textSize / 2f, paint);
            //重置画笔
            paint.reset();
