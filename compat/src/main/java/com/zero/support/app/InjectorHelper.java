...
public class InjectorHelper {
    ...
    public void onAttachFragment(InjectorViewModel viewModel, InjectFragment fragment) {
        ...
        messageEvent.observe(fragment, new Observer<Object>() {
            ...
            public void onChanged(@Nullable Object msg) {
                ...
                if (activity instanceof InjectorViewModel.MessageDispatcher) {
                    handled = ((InjectorViewModel.MessageDispatcher) activity).dispatchMessage(msg);
                }
                if (!handled) {
                    ActivityInjector.sendMessage(msg);
                }
            }
        });
    }
    ...
    protected void dispatchDialogEvent(DialogModel model) {
        ...
        if (activity instanceof InjectorViewModel.DialogCreator) {
            dialog = ((InjectorViewModel.DialogCreator) activity).onCreateDialog(model, activity);
        }
        if (dialog == null) {
