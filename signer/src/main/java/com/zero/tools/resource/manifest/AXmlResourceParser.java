...
import com.zero.tools.resource.StringBlock;
...
public class AXmlResourceParser implements XmlResourceParser {
    ...
    public AXmlResourceParser(ByteBuffer stream) {
        this();
        open(stream);
    }

    public ResourceException getFirstError() {
        return mFirstError;
    }
...
//    }
    ...
    public void setInput(InputStream stream, String inputEncoding) {
        ...
        open(ByteBuffer.wrap(outputStream.toByteArray()).order(ByteOrder.LITTLE_ENDIAN));
    }
    ...
    private final void doNext() throws IOException {
        ...
        if (m_strings == null) {
            DataUtil.skipCheckInt(m_reader,CHUNK_AXML_FILE, CHUNK_AXML_FILE_BROKEN);
        ...
        while (true) {
            ...
            if (event == START_DOCUMENT) {
                ...
                chunkType = m_reader.getInt();
            }

            if (chunkType == CHUNK_RESOURCEIDS) {
                int chunkSize = m_reader.getInt();
                if (chunkSize < 8 || (chunkSize % 4) != 0) {
                    throw new IOException("Invalid resource ids size (" + chunkSize + ").");
                }
                m_resourceIDs = DataUtil.readIntArray(m_reader,chunkSize / 4 - 2);
                continue;
            }
            ...
            // Common header.
			/* chunkSize */m_reader.getInt();
            int lineNumber = m_reader.getInt();
			/* 0xFFFFFFFF */m_reader.getInt();

            if (chunkType == CHUNK_XML_START_NAMESPACE || chunkType == CHUNK_XML_END_NAMESPACE) {
                if (chunkType == CHUNK_XML_START_NAMESPACE) {
                    int prefix = m_reader.getInt();
                    int uri = m_reader.getInt();
                    m_namespaces.push(prefix, uri);
                } else {
					/* prefix */m_reader.getInt();
					/* uri */m_reader.getInt();
                    m_namespaces.pop();
                }
                continue;
            ...
            if (chunkType == CHUNK_XML_START_TAG) {
                m_namespaceUri = m_reader.getInt();
                m_name = m_reader.getInt();
				/* flags? */m_reader.getInt();
                int attributeCount = m_reader.getInt();
                m_idAttribute = (attributeCount >>> 16) - 1;
                attributeCount &= 0xFFFF;
                m_classAttribute = m_reader.getInt();
                m_styleAttribute = (m_classAttribute >>> 16) - 1;
                m_classAttribute = (m_classAttribute & 0xFFFF) - 1;
                m_attributes = DataUtil.readIntArray(m_reader,attributeCount * ATTRIBUTE_LENGTH);
    ...
    private void setFirstError(ResourceException error) {
        if (mFirstError == null) {
            mFirstError = error;
        }
    }
	 ...
	 * m_strings.
	 */
    private ByteBuffer m_reader;
    private ResAttrDecoder mAttrDecoder;
    private ResourceException mFirstError;
